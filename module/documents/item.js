export class JuinkItem extends Item {
  prepareData() {
    ...
    if (
      this.actor != null &&
      this.actor.type == "scenario" &&
      this.system.life.add
    )
  ...
  async use(actor, chatMessage) {
    ...
    if (type == "addDice") {
      ...
      else action(this.system.effect.value);
      ...
      let updates = {};
      if (type == "addHope") {
        updates["system.hope.value"] =
          actor.system.hope.value + Number(this.system.effect.value);
      } else if (type == "addLife") {
        let r = new Roll(this.system.effect.value);
        await r.roll();
        r.toMessage();
        updates["system.life.value"] =
          actor.system.life.value + r.total > actor.system.life.max
            ? actor.system.life.max
            : actor.system.life.value + r.total;
      }
  ...
  async payCost(actor) {
    if (this.type == "ability") return await this._payCostForAbility();
    else if (this.type == "item") return await this._payCostForItem();
    else if (this.type == "event") return await this._payCostForEvent(actor);
  }

  async _payCostForAbility() {
    let type = this.system.link
      ? this.system.cost.linked.type
      : this.system.cost.normal.type;
    let value = this.system.link
      ? this.system.cost.linked.value
      : this.system.cost.normal.value;
    ...
    if (updates["system.hope.value"] < 0) return false;
  ...
  async _payCostForItem() {
    if (this.system.quantity.value < 1 || this.actor.system.itemUsage)
      return false;
    await this.update({
      "system.quantity.value": this.system.quantity.value - 1,
    });
  ...
  async toMessage() {
    ...
    let content = `<div class="juink-item-info" data-actor-id=${this.actor.id} data-item-id=${this.id}><h2 class="header">${title}</h2>`;

    if (this.type == "event") content += await this._getEventContent();
    else if (this.type == "identity")
      content += await this._getIdentityContent();
  ...
  async _getEventContent() {
    let content = `
      <div class="life-bar" style="${
        this.system.life.value >= this.system.life.max
          ? "background: green;"
          : Handlebars.compile("{{life value max}}")({
              value: this.system.life.value,
              max: this.system.life.max,
            })
      }">
          <div class="life">
              <input type="text" name="system.life.value" value="${
                this.system.life.value
              }" data-dtype="Number"/>
              <span> / </span>
              <input type="text" value="${
                this.system.life.max
  ...
  async _getIdentityContent() {
    let content = `
      <table>
        <tr>
          <td colspan="2"><b>${game.i18n.localize("Juink.Scene")}:&nbsp&nbsp</b>
          ${this.system.timing}</td>
        </tr>
        <tr>
          <td colspan="2"><b>${game.i18n.localize("Juink.Cost")}:&nbsp&nbsp</b>
          ${this.system.cost}</td>
        </tr>
        <tr>
          <td colspan="2"><b>${game.i18n.localize(
            "Juink.Target"
          )}:&nbsp&nbsp</b>
          ${this.system.target}</td>
        </tr>
        <tr>
          <td colspan="2"><b>${game.i18n.localize("Juink.Limit")}:&nbsp&nbsp</b>
          ${Handlebars.compile("{{abilityLimit arg}}")({
            arg: this.system.limit,
          })}</td>
        </tr>
      </table>
      ${this.system.description}
