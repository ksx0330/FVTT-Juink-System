...
export class JuinkScenarioSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      tabs: [
        {
          navSelector: ".tabs",
          contentSelector: ".scenario-contents",
          initial: "event",
        },
      ],
  ...
  async getData(options) {
    ...
    for (let i of data.items) {
      if (i.type == "event") {
        ...
        else actorData.otherEventList.push(i);
      } else if (i.type == "identity") {
        if (i.system.obtain) actorData.selectIdentityList.push(i);
        else actorData.otherIdentityList.push(i);
      }
    }
  ...
  async activateListeners(html) {
    ...
    html.find(".echo-item").click(async (event) => {
    ...
    html.find(".fate-dices .title").on("mousedown", async (event) => {
      ...
      else await this.document.chargeDice();
    });
    html.find(".dice").on("mousedown", async (event) => {
      let pos = $(event.currentTarget)[0].dataset.pos;
      if (event.button == 2 || event.which == 3) this.document.useDice(pos);
      else this.document.changeDice(pos);
    });
  }
